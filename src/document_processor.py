        ...
        try:
            ...
            if is_pdf:
                try:
                    ...
                    print(f"[DOCLING] {filename}: pages={num_pages}, "
                          f"texts={num_texts}, tables={num_tables}, pictures={num_pictures}")

                    # --- Secure rule ---
                    if num_texts == 0 and num_tables == 0 and num_pictures == 0:
                        print(f"[ROUTING] Docling empty → switching to PyPDF text only.")
                        dl_doc = None
                    ...
                    print(f"[ROUTING] Docling crashed → PyPDF fallback: {e}")
                    dl_doc = None
                    markdown_content = pypdf_to_markdown(str(original_path))

            # -----------------------------------------------------------------------
            #     Non-PDF or other formats → normal Docling
            # -----------------------------------------------------------------------
            else:
                result = self.converter.convert(str(original_path))
                dl_doc = result.document
            ...
            if dl_doc is not None:
                try:
                    (doc_dir / "document.json").write_text(
                        dl_doc.model_dump_json(indent=2), encoding="utf-8"
                    )
                except Exception as e:
                    print(f"⚠ Could not save JSON for {filename}: {e}")
            ...
            documents.append(
                Document(
                    ...
                    metadata={
                        ...
                        "file_type": uploaded_file.type,
                        "source": filename,
                        "extraction_mode": "docling" if dl_doc else "pypdf",
                        "output_dir": str(doc_dir),
                    },
                )
            )
            ...
            if dl_doc:
                docling_docs.append({"filename": filename, "doc": dl_doc, "pdf_path": str(original_path)})

            print(f"✅ {filename} processed OK")
            print(f"   → Markdown:{doc_dir/'document.md'}")
            print(f"   → JSON:{doc_dir/'document.json'}" if dl_doc else "   → NO Docling schema (PyPDF mode)")

        except Exception as e:
            print(f"❌ Failed {filename}: {e}")
            continue

    print(f"\n🎉 All files processed: {len(documents)}")
    return documents, docling_docs
