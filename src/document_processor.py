    ...
    def process_uploaded_files(self, uploaded_files) -> tuple[List[Document], List[Any]]:
        ...
        for uploaded_file in uploaded_files:
            ...
            try:
                ...
                # -------------------------------------------------------------------
                #  PDFs: try Docling first, fall back when it has NO structure
                # -------------------------------------------------------------------
                if is_pdf:
                    try:
                        result = self.converter.convert(str(original_path))
                        ...
                        # ✅ Secure rule: Docling "failed" if NO structure at all
                        if num_texts == 0 and num_tables == 0 and num_pictures == 0:
                            print(
                                "[ROUTING] Docling empty → switching to PyPDF text only."
                            )
                            dl_doc = None
                ...
                # -------------------------------------------------------------------
                #  Non-PDFs (docx, pptx, html, etc.): normal Docling
                # -------------------------------------------------------------------
                else:
                    result = self.converter.convert(str(original_path))
                    dl_doc = result.document
